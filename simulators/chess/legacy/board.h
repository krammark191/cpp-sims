...
public:
   ...
   virtual bool isEmpty(const Position& pos) const;
   virtual bool isEnPassantTarget(const Position& pos) const;
   virtual Position findKing(bool white) const;
   virtual bool isSquareAttacked(const Position& pos, bool byWhite) const;
   virtual bool isInCheck(bool white) const;
   virtual bool isMoveLegal(const Move& move) const;
   virtual void setOutput(ogstream * output) { pgout = output; }
...
private:
   ...
   void moveUnchecked(const Move& move, int currentMove);
};
